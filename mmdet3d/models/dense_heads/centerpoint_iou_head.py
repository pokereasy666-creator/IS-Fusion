...
class CenterIoUHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(CenterIoUHead, self).__init__(init_cfg=init_cfg)
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, preds_dicts):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            task_reg_preds.append(batch_reg_preds)

        batch_size = len(gt_bboxes_3d)

        task_reg_preds_list = [[] for ii in range(batch_size)]
        
        # convert to [batch_num, task_num, bbox] shape
        for batch_id in range(batch_size):
            task_reg_preds_list[batch_id] = [reg_preds[batch_id] for reg_preds in task_reg_preds]

        heatmaps, anno_boxes, inds, masks, iou_labels = multi_apply(
            self.get_targets_single, gt_bboxes_3d, gt_labels_3d, task_reg_preds_list)
        # Transpose heatmaps
        ...
        # Transpose inds
        iou_labels = list(map(list, zip(*iou_labels)))
        iou_labels = [torch.stack(iou_labels_) for iou_labels_ in iou_labels]
        return heatmaps, anno_boxes, inds, masks, iou_labels

    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, task_reg_preds):
        ...
        heatmaps, anno_boxes, inds, masks, iou_labels = [], [], [], [], []

        for idx, (task_head, reg_pred) in enumerate(zip(self.task_heads, task_reg_preds)):
            ...
            # compute overlaps between prediction and GTs
            if task_boxes[idx].shape[0] != 0:
                from mmdet3d.core.bbox.iou_calculators import axis_aligned_bbox_overlaps_3d, bbox_overlaps_3d
                iou_overlaps = bbox_overlaps_3d(reg_pred, task_boxes[idx])
                iou_targets, _ = torch.max(iou_overlaps, dim=-1)
            else:
                iou_targets = gt_labels_3d.new_zeros(reg_pred.shape[0], dtype=torch.float32)
