...
class ISFusionEncoder(BaseModule):
    ...
    def img_point_sampling(self, reference_voxel, mlvl_feats, num_cam=6, batch_size=4, **kwargs):
        ...
        for b in range(batch_size):
            ...
            # imgaug
            if isinstance(cur_img_aug_matrix, list): cur_img_aug_matrix = torch.stack([x for x in cur_img_aug_matrix])
            if cur_img_aug_matrix.dim() == 2: cur_img_aug_matrix = cur_img_aug_matrix.unsqueeze(0)
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            for b in range(batch_size):
                for k in range(num_cam):
                    reference_points_cam_lvl = reference_voxel_cam[b][min(k, len(reference_voxel_cam[b])-1)].view(-1, 1, 2)
                    sampled_feat = F.grid_sample(feat[min(k, len(feat)-1)][b].unsqueeze(dim=0), reference_points_cam_lvl.unsqueeze(dim=0))  # feat: [24,256,32,88]; reference_points_cam_lvl: [24, num_query, 1, 2]
