...
def safe_build_norm_layer(cfg, num_features, postfix=''):
    try:
        # 第一道防线：常规拦截
        if cfg is None or getattr(cfg, 'get', lambda x: None)('type') is None or str(getattr(cfg, 'get', lambda x: None)('type')).lower() == 'none':
            return 'norm', nn.Identity()
        
        # 第二道防线：强行执行，失败兜底
        return _orig_build_norm_layer(cfg, num_features, postfix)
    except KeyError as e:
        # 只要报错信息里说找不到 None 相关的东西，直接给空层
        if 'None' in str(e):
            return 'norm', nn.Identity()
        raise e
...
try:
    ...
    # 使用通配符接收所有参数 (如 out_fp16, out_fp32, apply_to 等)
