...
class WaymoDataset(KittiDataset):
    ...
    def get_data_info(self, index):
        ...
        single_name = os.path.join(self.data_root,
                                    info['image']['image_path'])
        img_filename = []
        lidar2img_list = []
        rect = info['calib']['R0_rect'].astype(np.float32)
        for img_id in range(self.img_num):
            new_filename = single_name[:-13] + str(img_id) + single_name[-12:]
            img_filename.append(new_filename)
            Trv2c = info['calib']['Tr_velo_to_cam%d' % img_id].astype(np.float32)
            P0 = info['calib']['P%d'%img_id].astype(np.float32)
    ...
    def get_ann_info(self, index):
        ...
        camera_ids = np.array(annos['camera_id']).reshape(-1, 1)
        gt_bboxes = np.concatenate([gt_bboxes, camera_ids], axis=-1)
        ...
        for cat in gt_names:
            if cat in self.CLASSES:
                ...
                gt_labels.append(3)     # NOTE set as background
    ...
    def format_results(self,
        ...
        # HACK here
        if 'img_bbox' in outputs[0]:
            del outputs[0]['img_bbox']
