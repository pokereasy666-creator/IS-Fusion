...
class CenterPointBBoxCoder(BaseBBoxCoder):
    ...
    def decode(self,
        ...
        out_size_factor = self.out_size_factor if type(self.out_size_factor) == int \
                            else self.out_size_factor[level_num]
        xs = xs.view(
            batch, self.max_num,
            1) * out_size_factor * self.voxel_size[0] + self.pc_range[0]
        ys = ys.view(
            batch, self.max_num,
            1) * out_size_factor * self.voxel_size[1] + self.pc_range[1]
