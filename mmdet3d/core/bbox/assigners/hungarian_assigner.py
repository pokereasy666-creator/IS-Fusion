...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg):
        ...
        cls_pred_T = cls_pred[0].T.sigmoid()  # 必须加 sigmoid！防止负数求 log 爆 NaN
        ...
        matched_row_inds = torch.as_tensor(matched_row_inds).to(bboxes.device)
        matched_col_inds = torch.as_tensor(matched_col_inds).to(bboxes.device)
...
class HungarianAssigner3DV3(BaseAssigner):
    ...
    def assign(self,
        ...
        matched_col_inds = torch.as_tensor(matched_col_inds, dtype=torch.long, device=assigned_labels.device)
        matched_row_inds = torch.as_tensor(matched_row_inds).to(
            bbox_pred.device)
        matched_col_inds = torch.as_tensor(matched_col_inds).to(
            bbox_pred.device)
