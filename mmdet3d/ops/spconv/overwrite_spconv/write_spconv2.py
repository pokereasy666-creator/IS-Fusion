...
try:
    ...
    # 兼容 MMCV 2.x：如果找不到，先试着从 mmcv.cnn 找，还找不到就造个假的
    try:
...
def register_spconv2():
    ...
    try:
        ...
        # 定义需要注册的算子列表
        spconv_layers = [
            (SparseConv2d, 'SparseConv2d'), (SparseConv3d, 'SparseConv3d'), (SparseConv4d, 'SparseConv4d'),
            (SparseConvTranspose2d, 'SparseConvTranspose2d'), (SparseConvTranspose3d, 'SparseConvTranspose3d'),
            (SparseInverseConv2d, 'SparseInverseConv2d'), (SparseInverseConv3d, 'SparseInverseConv3d'),
            (SubMConv2d, 'SubMConv2d'), (SubMConv3d, 'SubMConv3d'), (SubMConv4d, 'SubMConv4d')
        ]
        
        for layer_cls, layer_name in spconv_layers:
            try:
                # 优先尝试新版公开接口
                if hasattr(CONV_LAYERS, 'register_module'):
                    CONV_LAYERS.register_module(module=layer_cls, name=layer_name, force=True)
                # 兼容旧版私有接口
                elif hasattr(CONV_LAYERS, '_register_module'):
                    CONV_LAYERS._register_module(layer_cls, layer_name, force=True)
            except Exception:
                pass
