...
new_func = """    def extract_img_feat(self, img, img_metas):
        ...
        # 3. 穿透 DataContainer 提取 img Tensor 并推送到 GPU
        def _unwrap(x):
        ...
        if len(img_list) > 0:
            # 核心修复：必须将合并后的图像推送到 GPU！
            img = torch.cat(img_list, dim=0).to(device)
        else:
            img = None
...
print("✅ img 数据已成功关联 GPU Device！跨设备报错(CPU vs CUDA)已修复！")
